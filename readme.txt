=== SearchIQ - The Search Solution  ===
Contributors: searchiq, igorbelykh86, saurabhjaluka, findmsharma
Tags: Search,Site Search, Better search, Autocomplete, Ajax search, Analytics ,Custom Search,Search Engine ,Relevant search ,Highlight terms , Category tag author search, searchIQ
Donate link: http://www.searchiq.co/
Requires at least: 3.7
Tested up to: 6.4.3
Stable tag: 4.6
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Our FREE plugin makes your website’s search fast and more relevant. searchIQ helps you to manage content more effectively with real-time analytics.
== Description ==

SearchIQ provides an enhanced search experience to your WordPress site by delivering fast and accurate search results as you type. Your SearchIQ account also includes a realtime analytics dashboard that helps you understand your users’ search intent so you can optimize your content strategy to help your site grow.

Scalable from tens to millions of documents, SearchIQ powers sites ranging from small blogs to large enterprises.

## KEY Features

* Easy to setup - just minutes to implement
* Customizable UI
* Results optimization to prioritize relevant content
* AI to handle typos, synonyms, fuzzy matches, etc. 
* Search against custom fields and taxonomies
* A passionate, world-wide support staff available (nearly all hours) at support@searchiq.co

## Pricing

SearchIQ is absolutely free to get started with.   For businesses in need of advanced features such as PDF, Facet, CDN or custom branding; we offer simple and affordable packages that are easy to turn on. For more information on premium features, visit [pricing page](https://www.searchiq.co/pricing.html)

## Installation Video

[youtube="https://www.youtube.com/watch?v=barL-u6t7Ok"]

## Demo Site

[Demo site link](https://demo.searchiq.co)

## More Information

Visit [http://www.SearchIQ.co](http://www.SearchIQ.co) or contact us at contact@searchiq.co

== Installation ==

1. Create an account on SearchIQ.
2. Copy the API key from SearchIQ account.
3. Once you've created an account, go to your WordPress site and install SearchIQ WordPress plugin.
4. You will be asked to enter your searchIQ API key.
5. Select the post types you want to search.
6. Index your posts, you are all set!

If you are facing any issues then please email at support@searchiq.co

== Frequently Asked Questions ==

= Where can I find the API key? =
Login to your dashboard at www.searchiq.co and select “Interface” from the menu on the left.  You will find your api key at the bottom of the page.

= Why isn’t SearchIQ working for my website? =
Assuming you have installed everything according the instructions, there could be two reasons you still can’t see SearchIQ working as planned:

1. Your site doesn’t have a search bar -  In this case, go to options tab in the SearchIQ plugin admin. You will see options to add a search bar as a widget or as an icon in your site’s menu.

2. If your site does have a search bar - The search box input element name could be something other than the default value of 's'. Inspect the search box element on your site and copy the name into the "Search box name" field found under the "Options" tab in SearchIQ plugin. 

If you are still facing a problem, please drop us an email at support@searchiq.co.

= What about support? =
If you face an issue and need help, just send us an email to support@searchiq.co. Our support team will get back to you as soon as possible.

== Screenshots ==

1. SearchIQ on fashion site sample
2. SearchIQ facet feature sample

== Changelog ==
= 4.6 =
* Removed functionality to save errors in searchiq error log file. Using DB to log errors instead.

= 4.5 =
* Removed searchiq settings public call

= 4.4 =
* Rolled back the change to native search term filtering

= 4.3 =
* Fixed setting issue for partners
* Updated settings update for partners and non-partners
* Tested compatibility with latest Wordpress version

= 4.2 =
* Added Partner Support
* Fixed SearchIQ CSS not laoding on page with shortcode
* Added Wordpress WCAG changes
* Hardened potential search vulnerability areas

= 4.1 =
* Fixed issue with facets saving on custom post type
* Fixed issue with post synchronization in case of connection failure

= 4.0 =
* Fixed issue with product variation getting saved for draft products
* Optimised reset configuration functionaity.
* Improved configuration page dropdown load time via ajax
* Fixed bug related to DateTimeZone format.

= 3.10.2 =
* Added feature to continue full sync on error if all batches are not processed yet
* Logged failed batches in DB
* Improved Delta sync
* Added option to change Posts per page for full and delta sync. 
* Added option to change default API call timeout.

= 3.10.1 =
* Fixed issue related to image cache clearing on full sync
* Optimised full sync queries
* Optimised configuration page dropdown loading. Dropdowns load using ajax now.
* Added error log table for logging errors from API

= 3.10 =
* Fixed issue related to woocommerce addons compatibility

= 3.9 =
* fixed Xss related nononce issue, sanitized custom css field, removed nononce
* fixed notices that were generated while installing the plugin for first time
* removed cron trigger
* fixed escape and sanitization issues in the plugin

= 3.8 =
* fixed parenthesis issue for PHP versions > 7.4
* fixed deprecated jQuery code for latest wordpress versions

= 3.7 =
* fixed jQuery .size() usage to .length

= 3.6 =
* removed facet limit of 5
* added a setting to return multiple match in description

= 3.5 =
* improved post sync functionality

= 3.4 =
* removed unnecessary logging

= 3.3 =
* fixed duplicate indexing requests issue

= 3.2 =
* removed unnecessary file load
* fixed exception issue after plugin update

= 3.1 =
* Performance optimization

= 3.0 =
* Fixed issues with blacklist feature
* Fixed issues with Private(Hidden) products indexing in woocommerce
* Fixed issue on Author & Date setting for Pro users

= 2.9 =
* bug fix to index all the meta data

= 2.8 =
* bug fix for out of stock products

= 2.7 =
* bug fix while saving settings

= 2.6 =
* bug fix for unnecessary warn logs

= 2.5 =
* handled out of stock products status
* added an option to show/hide out of stock products

= 2.4 =
* Bug fixes

= 2.3 =
* Fix for broken built-in search page

= 2.2 = 
* Fix for broken search page

= 2.1 =
* Absolute and relative url fix for images and pdfs.
* Fixed the thousands separator issue for e-commerce sites

= 2.0 =
* upgrade in the search algorithm
* migrated to searchiq.co
* bug fixes

= 1.5.7 =
* bug fixes

= 1.5.6 =
* bug fix for delta sync
* added a fromDate & toDate parameter for delta sync api

= 1.5.5 =
* added feature to do delta sync
* added api to do delta sync for direct db change
* improved thumbnail generation sync process

= 1.5.4 =
* added shortcode option to add a search bar (see options tab)

= 1.5.3 =
* minor bug fix
* index improvements

= 1.5.2 =
* php 7 error fix
* fixed hidden products indexing bug
* added default thumbnail option if no image is found in the options tab
* bug fixes

= 1.5.1 =
* added searchIQ Plus feature to hide searchIQ logo - only visible to paid user
* minor UI fixes
* fixed notices issue

= 1.4.17 =
* For pro user - added support for shortcode to add search bar or search icon
* For pro user - added option for post type selection in search widget

= 1.4.16 =
* added an option to open results in new tab under options tab
* fixed warning notices

= 1.4.15 =
* fixed post synchronization and UI bugs

= 1.4.14 =
* fixed bugs

= 1.4.13 =
* added cross site search to mobile version
* improved performance
* fixed bugs

= 1.4.12 =
* added option for thumbnail generation: crop or resize

= 1.4.11 =
* added a feature to change the default sort by (Relevance, Newest) - visit options tab to see a sort by option
* fixed mobile search enable/disable issue
* fixed mobile search to search on configured post types and not just posts

= 1.4.10 =
* bug fix (removed unicode filtering code)

= 1.4.9 =
* resolved font conflict issue for icomoon

= 1.4.8 =
* launched mobile search
* bug fixes

= 1.4.7 =
* post count bug fix
* warnings bug fix
* ui fixes

= 1.4.6 =
* added more text customization option
* bug fixes

= 1.4.5 =
* bug fixes

= 1.4.4 =
* bug fixes

= 1.4.3 =
* bug fixes

= 1.4.2 =
* bug fixes

= 1.4.1 =
* bug fixes

= 1.4.0 =
* added support for search on custom fields
* added an option to add search to widget and/or menu
* simplified plugin configuration process
* bug fixes

= 1.3.2 =
* bug fixes

= 1.3.1 =
* fixed sync issue

= 1.3.0 =
* added option to select search algorithm exact match or broad match
* bug fixes

= 1.2.2 =
* fixed bugs

= 1.2.1 =
* added some customization for search results

= 1.2.0 =
* released mobile search
* bug fixes

= 1.1.5 =
* resolved plugin settings issue

= 1.1.4 =
* resolved conflict issue with autoptimize plugin

= 1.1.3 =
* https bug fix

= 1.1.2 =
* fixed bugs
* performance improvement
* new search result layout

= 1.1.1 =
* fixed delete/trash a post bug

= 1.1.0 =
* changed js & css request to non blocking call
* added O.S, Country, State, Browser details in dashboard
* combined cross domain search results into one results
* improved plugin performance

= 1.0.13 =
* added support for custom field image

= 1.0.12 =
* css sync fix
* added support to customize autocomplete UI text to any language

= 1.0.11 =
* faster custom search page redirect
* css override sync
* stable custom search page layout

= 1.0.10 =
* thumbnail bug fix
* https bug fix
* added css override in the options tab
* added support for indexing post with empty content

= 1.0.9 =
* bug fix

= 1.0.8 =
* bug fixes
* added support for https

= 1.0.7 =
* bug fixes and added support for cross site search

= 1.0.6 =
* added capability to support themes that has search box name other than 's'
* bug fixes

= 1.0.5 =
* fixed thumbnail generation process
* other bug fixes

= 1.0.4 =
* UI enhancement
* Resolved jquery conflict issue
* bug fixes

= 1.0.3 =
* Bug fixes

= 1.0.1 =
* Indexing performance improvement
* Bug fixes

= 1.0 =
* Initial Release
