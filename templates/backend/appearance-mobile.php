...
if(!empty( $_POST ) && isset($_POST['btnSubmitMobileStyleOptions']) && check_admin_referer($this->updateMobileNonce)){
		...
		if(isset($_POST['search_icon_selector']) && !empty($_POST['search_icon_selector'])){	
			update_option($this->pluginOptions['search_icon_selector'], sanitize_text_field( $_POST['search_icon_selector'] ) );
		}else{
...
<div class="wsplugin">
 ...
 <form method="POST" novalidate action="<?php _e(esc_url( admin_url( 'admin.php?page=dwsearch&tab=tab-5') ) ); ?>" onsubmit="return SIQ_validateMobileSettingsForm();" class="custom_page_options" name="custom_options">
	<div class="section section-1">
        ...
        <div class="data">
            <label>Enable</label>
            <input type="checkbox" name="mobile_enabled" value="yes" <?php esc_html_e( $this->isMobileEnabled() ? "checked=checked": "");?> />
        </div>

        <div class="data">
            <label>Search Bar Placeholder</label>
            <input type="text" value="<?php esc_html_e( $styling['barPlaceholder'] );?>" class="long-input" name="styling[barPlaceholder]"/>
        </div>
        <div class="data">
            <label>Search Result Header Text</label>
            <input type="text" value="<?php esc_html_e( $styling['SearchHeadTxt'] );?>" class="long-input" name="styling[SearchHeadTxt]"/>
        </div>
        <div class="data">
            <label>Search Result Stat Text</label>
            <input type="text" value="<?php esc_html_e( $styling['SearchResultStatTxt'] );?>" class="long-input" name="styling[SearchResultStatTxt]"/>
        </div>
        <div class="data">
            <label>Search Result Loading text</label>
            <input type="text" value="<?php esc_html_e( $styling['SearchLoadingTxt'] );?>" class="long-input" name="styling[SearchLoadingTxt]"/>
        </div>
        <div class="data">
            <label>Search Bar Placeholder Text Color</label>
            <input type="text" value="<?php esc_html_e( $styling['barPlaceholderTextColor'] );?>" name="styling[barPlaceholderTextColor]" class="color {required:false}"/>
        ...
        <div class="data">
            ...
            <select class="auto-width" id="mobile_favicon_mode" name="mobile_favicon_mode" onchange="var custom = document.getElementById('mobileSearchBarFavicon'); this.value == '2' ? custom.setAttribute('type', 'text') : custom.setAttribute('type', 'hidden');">
                <option value="0" <?php esc_html_e( $this->pluginSettings['mobile_favicon_mode'] == 0 ? "selected" : "" );?>>Disable</option>
                <option value="1" <?php esc_html_e( $this->pluginSettings['mobile_favicon_mode'] == 1 ? "selected" : "" );?>>Auto-detect</option>
                <option value="2" <?php esc_html_e( $this->pluginSettings['mobile_favicon_mode'] == 2 ? "selected" : "" );?>>Custom URL</option>
            </select>
            <input class="long-input" id="mobileSearchBarFavicon" type="<?php esc_html_e( $this->pluginSettings['mobile_favicon_mode'] == 2 ? "text" : "hidden" );?>" name="styling[barFavicon]" value="<?php esc_html_e( $styling['barFavicon'] );?>"/>
        </div>
        <div class="data">
			<label>Search Bar Background</label>
			<input type="text" value="<?php esc_html_e( $styling['barBgColor'] ); ?>" name="styling[barBgColor]" class="color {required:false}"/>
			<div class="clearColor"><span></span></div>
		</div>
        <div class="data">
            <label>Search Input Background</label>
            <input type="text" value="<?php esc_html_e( $styling['barInputBgColor'] );?>" name="styling[barInputBgColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Search Input Text Color</label>
            <input type="text" value="<?php esc_html_e( $styling['barInputTextColor'] );?>" name="styling[barInputTextColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
        ...
        <div class="data">
            ...
            <input type="checkbox" name="mobile_float_bar_enabled" value="yes" <?php esc_html_e( $this->isMobileFloatBarEnabled() ? "checked=checked" : "" );?> />
        </div>

        <div class="data">
            <label>Enable Float Search Icon</label>
            <input type="checkbox" name="mobile_icon_enabled" value="yes" <?php esc_html_e( $this->isMobileIconEnabled() ? "checked=checked" : "" );?> />
        </div>
		
		<div class="data">
            <label>Search icon selector</label>
            <input type="text" name="search_icon_selector" value="<?php esc_html_e( $settings['search_icon_selector'] ); ?>" />
        </div>
		
        <div class="data">
            <label>Search icon box background</label>
            <input value="<?php esc_html_e( $styling['searchIconBoxBg'] ? $styling['searchIconBoxBg'] : $this->mobileStyling['searchIconBoxBg']['default'] ); ?>" name="styling[searchIconBoxBg]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Search icon color</label>
            <input value="<?php esc_html_e( $styling['searchIconColor'] ? $styling['searchIconColor'] : $this->mobileStyling['searchIconColor']['default'] ); ?>" name="styling[searchIconColor]" class="color {required:false}"/>
            <div class="clearColor"><span></span></div>
        </div>
        <div class="data">
            <label>Position offset from top</label>
            <input value="<?php esc_html_e( $styling['searchIconTopOffset'] );?>" type="number" name="styling[searchIconTopOffset]" onblur="if(!/(^(\d+\.?)(\d+)?$|^(\d+)?(\.\d+)$)/.test(this.value)){this.value=this.value.replace(/[^\d\.]*/, '');}"/>
            <select name="styling[searchIconTopOffsetUnit]" style="max-width: 50px;top: -3px;position: relative;">
                <option <?php esc_html_e( $styling['searchIconTopOffset'] && $styling['searchIconTopOffsetUnit'] == "%" || !$styling['searchIconTopOffset'] && $this->mobileStyling['searchIconTopOffsetUnit']['default'] == "%" ? "selected" : "" );?> value="%">%</option>
                <option <?php esc_html_e( $styling['searchIconTopOffset'] && $styling['searchIconTopOffsetUnit'] == "px" || !$styling['searchIconTopOffset'] && $this->mobileStyling['searchIconTopOffsetUnit']['default'] == "px" ? "selected" : "" );?> value="px">px</option>
            </select>
        </div>
	</div>
