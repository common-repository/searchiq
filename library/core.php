...
class siq_core{
	...
	const DEFAULT_DESCRIPTION_SEGMENTS = 1;
	...
	public $metaFieldsSkipped = array('_edit_lock','_edit_last','_encloseme','_wp_attachment_metadata','_wp_attached_file', '_thumbnail_id','_wp_trash_meta_status','_wp_trash_meta_time','_wp_desired_post_slug','_wp_old_slug','_wp_old_date','_pingme');
    ...
    private $allCustomFields = array();
	public $kses_allowed_html_searchbox = array( 'select' => array('name'=>array(), 'id'=> array(), 'class'=>array(), "disabled" => array(), 'style' => array() ), 'option' => array('value' => array(), 'selected' => array(), 'style' => array() ), 'optgroup' => array('value' => array(), 'label' => array(), 'style' => array() ) );
	public $kses_allowed_html_config = array( 
										'select' => array('name'=>array(), 'id'=> array(), 'class'=>array(), "disabled" => array(), 'style' => array() ), 
										'option' => array('value' => array(), 'selected' => array(), 'style' => array() ),
										'optgroup' => array('value' => array(), 'label' => array(), 'style' => array() ),
										'table' => array('class'=>array(), 'id'=>array(), 'width'=>array(), 'cellpadding'=> array(), 'cellspacing'=>array(), 'border'=>array(), 'style' => array() ),  
										'tbody' => array('class'=>array(), 'id'=>array(), 'style' => array()  ),  
										'tr' => array('class'=>array(), 'id'=>array(), 'rowspan' => array(), 'style' => array(), 'data-posttype' => array()  ),  
										'th' => array('class'=>array(), 'id'=>array(), 'colspan' => array(), 'style' => array()  ),  
										'td' => array('class'=>array(), 'id'=>array(), 'colspan' => array(), 'style' => array()  ), 
										...
										'input' => array('class'=>array(), 'id'=>array(), 'name' => array(), 'value' => array(), 'type' => array(), 'style' => array(), 'placeholder' => array(), 'checked' => array(), 'disabled' => array() ), 
										'textarea' => array('class'=>array(), 'id'=>array(), 'name' => array(), 'rows' => array(), 'cols' => array(), 'style' => array(), 'placeholder' => array() ), 
										'b' => array(),
										'br' => array(),
										'a' => array('href'=> array(), 'target'=> array(), 'class'=> array(), 'id' => array() ),
										'span' => array('class'=> array(), 'id' => array() ),
										'label' => array('class'=> array(), 'id' => array() ),
										'form' => array('class'=> array(), 'id' => array(), 'action' => array(), 'method' => array() ),
										'script' => array('type'=> array()),
									);
	protected $taxomomyKeyMap	= array(
										'category' => 'categories', 
										'post_tag' => 'tags',
	...
	function __construct(){
		...
		$this->pluginOptions = array(
			...
			'is_partner' 					=> '_siq_is_partner',
			'partner_js_tag'				=> '_siq_partner_js_tag',
			'domain_list' 					=> '_siq_domain_list',
			'disable_sync'					=> '_siq_disable_sync',
			'raw_settings'					=> '_siq_raw_settings',
			...
			"descriptionSegments" => "_siq_descriptionSegments",
			"siq_sync_process_log" => "_siq_sync_process_log",
			"siq_full_sync_failed_batches" => "_siq_full_sync_failed_batches"
		);
	...
	public static function errorTableName(){
		global $wpdb;
		return $wpdb->prefix . 'siq_errors';
	}

	public function addToSync($postIDs, $deleteUnsynced = true){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		global $wpdb;	
		$result = array();
		$result["success"] = 0;
		...
		if( $deleteUnsynced === true ){
			$deletedPostIdsString	= $this->getSyncPostIds('d');
			if(!is_array($deletedPostIdsString) && !empty($deletedPostIdsString)){
		...
		if( is_array($postIDs) && count($postIDs) > 0 ) {
			...
			$query = "INSERT INTO `%5s` (`post_id`,`sync_time`) VALUES ";				
	...
	public function getSyncPostIds($status = 'd', $getCount = false, $limit = 0){
		...
		if($status == "d"){
			...
			$theQuery = "SELECT tbl.post_id FROM %5s tbl"; 
			$theQuery .=" LEFT JOIN  %5s tbl1 ON tbl.post_id=tbl1.id";
			...
			$theQuery = "SELECT tbl1.id as post_id FROM %5s tbl";
			$theQuery .=" RIGHT JOIN %5s tbl1 ON tbl.post_id=tbl1.id";
			$theQuery .=" WHERE ( tbl1.post_modified>tbl.sync_time $timelimitTbl1)";
	...
	public function removeFromSync($postID = ""){
		...
		$response = 0;
		$query = '';
		$idsForDeletion = false;
		$where = '';
		if(is_array($postID) && count($postID) > 0){
			$where =  "IN(".implode(',',$postID).") ";
			$idsForDeletion = true;
		}else if(!is_array($postID) && !empty($postID)){
			$where =  "=".$postID;
			$idsForDeletion = true;
		}
		if($idsForDeletion){
	...
	public function log_error( $error, $type = 'info', $onlySelected = false ) {
		if ( ( $this->logErrors && $type == 'error' ) || ( $this->logInfo && $type == 'info' ) || $onlySelected == true ) {
			$this->enableErrorLog( $this->logErrors, $this->logInfo);
			if( !empty( $error ) ){
				$CallUrl = '';
				$callMethod = strtoupper($type);
				$request_body = '['.gmdate("Y:m:d H:i:s").']: '.$error;
				$response_status_code = '';
				$response_body = '';
				$this->logAPIError($CallUrl, 
					$callMethod, 
					$request_body, 
					$response_status_code, 
					$response_body
	...
	public function enableErrorLog( $logError = false, $logInfo = false ) {
	...
	function getPluginSettings($param = ''){
		...
		if (isset($setting['mobile_enabled']) && is_null($setting['mobile_enabled'])) {
			$setting['mobile_enabled'] = self::DEFAULT_MOBILE_ENABLED;
			$this->setMobileEnabled(self::DEFAULT_MOBILE_ENABLED);
		}
		if (!isset( $setting['mobile_icon_enabled'] ) || ( isset($setting['mobile_icon_enabled']) && is_null($setting['mobile_icon_enabled']) ) ) $setting['mobile_icon_enabled'] = self::DEFAULT_MOBILE_ICON_ENABLED;
		if (!isset( $setting['mobile_float_bar_enabled'] ) || ( isset($setting['mobile_float_bar_enabled']) && is_null($setting['mobile_float_bar_enabled']) ) ) $setting['mobile_float_bar_enabled'] = self::DEFAULT_MOBILE_FLOAT_BAR_ENABLED;
		if (!isset( $setting['mobile_favicon_mode'] ) || ( isset($setting['mobile_favicon_mode']) && is_null($setting['mobile_favicon_mode']) ) ) $setting['mobile_favicon_mode'] = self::DEFAULT_MOBILE_FAVICON_MODE;
		if (!isset( $setting['siq_crop_resize_thumb'] ) || ( isset($setting['siq_crop_resize_thumb']) && is_null($setting['siq_crop_resize_thumb']) ) ) $setting['siq_crop_resize_thumb'] = self::DEFAULT_CROP_RESIZE_THUMB;
		if (!isset( $setting['siq_field_for_excerpt'] ) || ( isset($setting['siq_field_for_excerpt']) && is_null($setting['siq_field_for_excerpt']) ) ) $setting['siq_field_for_excerpt'] = "";
		if (!isset( $setting['featureExcludeFields'] ) || ( isset($setting['featureExcludeFields']) && is_null($setting['featureExcludeFields']) ) ) $setting['featureExcludeFields'] = false;
		if (!isset( $setting['isProPack'] ) || ( isset($setting['isProPack']) && is_null($setting['isProPack']) ) ) $setting['isProPack'] = false;
		if (!isset( $setting['siq_postTypesForSearchSelection'] ) || ( isset($setting['siq_postTypesForSearchSelection']) && is_null($setting['siq_postTypesForSearchSelection']) ) ) $setting['siq_postTypesForSearchSelection'] = "";
		if (!isset( $setting['siq_resultPageShowPostLink'] ) || ( isset($setting['siq_resultPageShowPostLink']) && is_null($setting['siq_resultPageShowPostLink']) ) ){ $setting['siq_resultPageShowPostLink'] = self::DEFAULT_POST_URL_ENABLED; }else{ $setting['siq_resultPageShowPostLink'] = isset($setting['siq_resultPageShowPostLink']) ? (int)$setting['siq_resultPageShowPostLink']: ''; }
		if (!isset( $setting['siq_resultPageCustomSearchItemLinkFontSize'] ) || ( isset($setting['siq_resultPageCustomSearchItemLinkFontSize']) && is_null($setting['siq_resultPageCustomSearchItemLinkFontSize']) ) ){ $setting['siq_resultPageCustomSearchItemLinkFontSize'] = self::DEFAULT_POST_URL_FONT_SIZE;}else{ $setting['siq_resultPageCustomSearchItemLinkFontSize'] = isset($setting['siq_resultPageCustomSearchItemLinkFontSize']) ? (int)$setting['siq_resultPageCustomSearchItemLinkFontSize']: ''; }
		if (!isset( $setting['siq_resultPageCustomSearchItemLinkColor'] ) || ( isset($setting['siq_resultPageCustomSearchItemLinkColor']) && is_null($setting['siq_resultPageCustomSearchItemLinkColor']) ) ) $setting['siq_resultPageCustomSearchItemLinkColor'] = self::DEFAULT_POST_URL_FONT_COLOR;
	...
	private function _siq_delete_post($postID, $post_type){
		...
		if( !empty( $engineKey ) ) {
			try{
				...
				$removed = $this->removeFromSync($postID);
				if( $removed > 0 ){
					$this->updateIndexCount('d', (int)$removed);
				}
				$this->deleteAttachments($postID);
				$childPosts = $this->getChildPosts($postID);
				if (is_array($childPosts) && count($childPosts) > 0) {
					$this->_submit_for_bulk_deletion($childPosts);
				}
			}catch(Exception $e){
	...
	private function deleteAttachments($postID){
		...
		$this->log_error("SIQ LOG: ATTACHMENT LIST: ('".json_encode($pdfs)."')");
		if(count($pdfs) > 0){
	...
	private function _siq_insert_update_post($postID, $post, $update = false){
		...
		if( ! empty ( $engineKey ) ) { 
			...
			try{
				...
				$this->checkIfProductAndVariation($post, $document, $postBulkDocuments);
						...
						if($postBulkDocuments == true){
							if($result['success'] == 1){
								...
								foreach($document as $k => $v){
									$this->addToSync($v["externalId"], false);
								}
							}
						}else{
							if($result['success'] == true && (strpos($result['message'], 'created') !== FALSE || strpos($result['message'], 'updated') !== FALSE)){
								if(strpos($result['message'], 'created') !== FALSE){
									$this->updateIndexCount('i');
								}
								$this->addToSync($postID, false);
							}
						}
					}
				}else{
	...
	private function postNotToBeIndexed($post, $url = ""){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
			$flagExempt = false;
			if($this->checkIfPostIsInvalidVariableProduct($post)){
				$flagExempt = true;
                $this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
                return $flagExempt;
			}
	...
	public function createDocumentFromPost(&$post, $single = true, &$taxomony = array() ){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$image				= ""; # TODO: looks like we are not using this variable. Check and remove.
		...
		$dataForSubmission["title"] 				= $this->handleSpecialChars($title);
        $body = html_entity_decode( wp_strip_all_tags( $this->strip_shortcodes_from_content( $post->post_content ) ), ENT_QUOTES, "UTF-8" );
		$dataForSubmission["body"] 					= $this->handleSpecialChars($body);
		$dataForSubmission["externalId"] 			= (int)$post->ID;
        $author = get_the_author_meta( 'display_name', $post->post_author );
		$authors	= array( $this->handleSpecialChars($author));
		...
		if( count( $taxomony ) == 0 ){ 
			...
			if(is_array($wpcats) && count($wpcats) > 0){
				foreach ($wpcats as $c) {
					$cat    = get_cat_name( $c );
					$cats[] = $this->handleSpecialChars($cat);
				}
			}
			...
			if( is_array( $wptags ) && count($wptags) > 0 ) {
				foreach( $wptags as $tag ) {
					$tagName = $tag->name;
					$tags[]  = $this->handleSpecialChars($tagName);
				}
			}
			...
			$dataForSubmission							= $this->processPostTaxonomies($post->ID, $taxomony, $excludeTaxonomies, $dataForSubmission);
		}
		$dataForSubmission["timestamp"]				= gmdate('Y-m-d\TH:i:s\.\0\0\0', strtotime($post->post_date_gmt));
		$excerptValue								= $this->handleSpecialChars($post->post_excerpt);
		$dataForSubmission["excerpt"] 				=  html_entity_decode( wp_strip_all_tags( $excerptValue), ENT_QUOTES, "UTF-8" );
		 ...
		 */
		if( count( $taxomony ) == 0 && $this->allTaxonomies!= "" && is_array($this->allTaxonomies) && count($this->allTaxonomies) > 0 ) {
				...
				if(!in_array($taxonomy, $excludeTaxonomies)) {
					...
					if ($terms != "" && count($terms) > 0) {
						foreach ($terms as $k => $v) {
							$termName = $v->name;
							$dataForSubmission[$this->customTaxonomyPrefix . $taxonomy][] = $this->handleSpecialChars($termName);
						}
					}
				}
		...
		if(isset($post->meta_key) && isset($post->meta_value)) {
			...
			if ($meta_keys !="" && is_array($meta_keys) && count($meta_keys) > 0) {
				foreach ($meta_keys as $key => $value) {
					if (isset($meta_values[$key]) && $meta_values[$key] != "" && !in_array($value, $this->metaFieldsSkipped) && !in_array($value, $excludeFields)) {
						if($this->woocommerceActive) {
							...
							if( count( $attributes ) > 0 ){
								foreach( $attributes as $attKey => $attValue ){
									$keyToCheck = preg_replace('/^'.$this->customAttributePrefix.'/', $this->customTaxonomyPrefix, $attKey);
									if( array_key_exists( $keyToCheck, $dataForSubmission) ){
										unset( $dataForSubmission[$keyToCheck] );
									}
								}
							}
						    ...
						    $cfv = $meta_values[$key];
							$dataForSubmission[$this->customFieldPrefix.$value] = $this->handleSpecialChars($cfv);
						}
					}
				}
			}
				...
				foreach ($postMeta as $k => $v) {
					if($v[0] != "" && !in_array($k, $this->metaFieldsSkipped) && !in_array($k, $excludeFields)) {
						if($this->woocommerceActive){
						    ...
						    $cfv_ = $v[0];
							$dataForSubmission[$this->customFieldPrefix.$k] = $this->handleSpecialChars($cfv_);
						}
					}
				}
			}
		...
		$this->createPermalinkForVariableProduct($dataForSubmission, $post);
        $this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
		return $dataForSubmission; 
	}
    ...
    private function processProductTaxonomies($taxonomy, &$dataForSubmission){
        ...
        switch ($taxonomy){
            case 'product_cat':
                $dataForSubmission['categories'] = isset( $dataForSubmission[$this->customTaxonomyPrefix . $taxonomy] ) ? $dataForSubmission[$this->customTaxonomyPrefix . $taxonomy] : array();
                break;
            case 'product_tag':
                $dataForSubmission['tags'] = isset( $dataForSubmission[$this->customTaxonomyPrefix . $taxonomy] ) ? $dataForSubmission[$this->customTaxonomyPrefix . $taxonomy] : array();
                break;
	...
	private function processWoocommerceAttributes($ID, $name, $value){
		...
		$attributes = array();
			switch($name){
				...
				default:
					$attributes[$this->customFieldPrefix.$name] = $this->handleSpecialChars($value);
			}
			$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
		return $attributes;
	}
					...
					if(!in_array($k, $excludeTaxonomies)) {
						if ($v['is_taxonomy'] == 1) {
							...
							if (is_array($termData) && count($termData) > 0) {
								array_walk($termData, array($this, 'handleSpecialChars'));
							}
							$attributes[$this->customAttributePrefix . $k] = $termData;
						} else if ($v['is_taxonomy'] == 0) {
							$attributes[$this->customAttributePrefix . $k] = $this->handleSpecialChars($v["value"]);
						} else {
							$attributes[$this->customAttributePrefix . $k] = $this->handleSpecialChars($v["value"]);
						}
					}
	...
	private function get_images_from_post($post) {
		...
		$origImageUrlDB 	= get_post_meta($post->ID, self::POST_ORIG_IMG_URL_META_KEY, true);
		$origImageUrlPost 	= $this->get_orig_post_image($post);
		if ($origImageUrlDB != $origImageUrlPost ) {
			$origImageUrl = $origImageUrlPost;
	...
	protected function getFieldForExcerpt($postType = ""){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$pluginSettingsICF = isset($this->pluginSettings["siq_field_for_excerpt"]) ? $this->pluginSettings["siq_field_for_excerpt"]: '';
		if($pluginSettingsICF != "" && strpos($pluginSettingsICF, ':') !== FALSE) {
			$pluginSettingArrICF = array();
			$pluginSettingArrICFFinal = array();
			if ($pluginSettingsICF != "") {
    ...
    protected function getExcludedCustomFields($postType = ""){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
        $pluginSettingsECF = isset($this->pluginSettings["exclude_custom_fields"]) ? $this->pluginSettings["exclude_custom_fields"] : '';
	...
	protected function getExcludedCustomTaxonomies($postType = ""){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$pluginSettingsECF = isset($this->pluginSettings["exclude_custom_taxonomies"]) ? $this->pluginSettings["exclude_custom_taxonomies"] : '';
		if($pluginSettingsECF != "" && strpos($pluginSettingsECF, ';') !== FALSE) {
			$pluginSettingsArrECF = array();
			$pluginSettingArrECFFinal = array();
			$pluginSettingArrECF = explode(';', $pluginSettingsECF);
    ...
    protected function getExcludedPostIds(){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
        $pluginExcludePosts = isset($this->pluginSettings["exclude_posts"]) ? $this->pluginSettings["exclude_posts"]: '';
	...
	private function get_orig_post_image($post) {
		...
		if (!empty($imgCustomField)) {
			...
			if(strpos($metaValue, '[') !== FALSE && strpos($metaValue, ']') !== FALSE){
				$metaValue = preg_replace('/\[(.*)\]/', '$1', $metaValue);
			}
			if (!empty($metaValue)) {
	...
	private function _submit_for_indexing(&$data, $total = 0){
		...
		if ($result['success'] == true) {
			...
			$total = (int)get_option($this->pluginOptions['num_indexed_posts']);
			$result['indexed'] = $total;
		} 
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
		return $result;
		...
		if( ! empty( $engineKey ) ) {
			try{
				...
				if($response['response_code'] != ""){
					...
					$countDeleted = array_key_exists('deleted', $response["response_body"]) ? $response["response_body"]["deleted"] : 0;
					if ($countDeleted  > 0) {
						$this->updateIndexCount('d', $countDeleted);
						if( array_key_exists('details', $response["response_body"]) ){
							$deleted = [];
							foreach( $response["response_body"]["details"] as $detail ){
								if( isset( $detail["deleted"]) && $detail["deleted"] == true ){
									array_push($deleted, (int)$detail["externalId"]);
								}
							}
							if( count( $deleted ) > 0 ){
								$this->removeFromSync( $deleted );
							}
						}
					}
					$this->log_error("response from api:success: result(".json_encode($response).")");
	...
	private function _delete_all_posts(){
		...
		if( $result['success'] === true ){ $this->truncateSyncTable(); }
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
		return $result;
	}
	...
	public function is_custom_search_page_set(){
		...
		if(($this->pluginSettings['use_custom_search'] == 'yes' || !empty( $siq_result ) && $siq_result == "on") && !empty($this->pluginSettings['custom_search_page'])){
	...
	protected function setPostTypesForSearch( $post = array() ) {
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$types = $this->sanitizeVariables($post['post_types']);
	...
	protected function _siq_get_sync_settings(){
		...
		global $siqAPIClient;
		$api_key 	= $siqAPIClient->siq_get_api_key();
		$engineKey	= isset($this->pluginSettings['engine_code']) ? $this->pluginSettings['engine_code']: '';
		if(!empty($engineKey)) {
			try {
				...
				if (!empty($response['response_code'])) {
					$result = $response['response_body'];
					if( isset( $response['response_string']) ){
						update_option($this->pluginOptions["raw_settings"], $response['response_string']);
					}
		...
		if(empty($engine_just_created)){
			try{
				$params['params'] = array(
					...
					"customSearchItemTitleFontSize"    => (isset($style['resultTitleFontSize']) && !is_null($style['resultTitleFontSize']) && (int)($style['resultTitleFontSize']) > 0) ? $style['resultTitleFontSize'] : null,
					"customSearchItemTitleColor"    => !empty($style['resultTitleColor']) ? $style['resultTitleColor'] : null,
					"customSearchItemBodyFontSize"    => (isset($style['resultTextFontSize']) && !is_null($style['resultTextFontSize']) && (int)($style['resultTextFontSize']) > 0) ? $style['resultTextFontSize'] : null,
					...
					"customSearchItemAuthorAndDateFontSize"    => (isset($style['resultAuthDateFontSize']) && !is_null($style['resultAuthDateFontSize']) && (int)($style['resultAuthDateFontSize']) > 0) ? $style['resultAuthDateFontSize'] : null,
					...
					"customSearchItemCategoryFontSize"    => (isset($style['resultCatTitleFontSize']) && !is_null($style['resultCatTitleFontSize']) && (int)($style['resultCatTitleFontSize']) > 0) ? $style['resultCatTitleFontSize'] : null,
					...
					"customSearchItemTagFontSize"    => (isset($style['resultTagFontSize']) && !is_null($style['resultTagFontSize']) && (int)($style['resultTagFontSize']) > 0) ? $style['resultTagFontSize'] : null,
					...
					"customSearchPaginationFontSize"    => (isset($style['paginationFontSize']) && !is_null($style['paginationFontSize']) && (int)($style['paginationFontSize']) > 0) ? $style['paginationFontSize'] : null,
					...
					"customCss"                     => $this->processStyling( ( isset($style['customCss']) && !is_null($style['customCss']) ) ? stripslashes($style['customCss']): ''),
					...
					"autocompleteWidth"             => ( isset($this->pluginSettings["autocomplete_style"]["autocompleteWidth"]) && (int) ($this->pluginSettings["autocomplete_style"]["autocompleteWidth"]) > 0 )? (int) $this->pluginSettings["autocomplete_style"]["autocompleteWidth"] : null ,
					...
					"autocompleteResultFontSize" => (isset($this->pluginSettings["autocomplete_style"]["resultFontSize"]) && (int)( $this->pluginSettings["autocomplete_style"]["resultFontSize"]) > 0)  ? $this->pluginSettings["autocomplete_style"]["resultFontSize"] : null ,
					...
					"autocompleteResultHighlightFontSize" => (isset($this->pluginSettings["autocomplete_style"]["highlightFontSize"]) && (int)( $this->pluginSettings["autocomplete_style"]["highlightFontSize"]) > 0)  ? $this->pluginSettings["autocomplete_style"]["highlightFontSize"] : null ,
					...
					"autocompleteResultHoverFontSize" => (isset($this->pluginSettings["autocomplete_style"]["hoverResultFontSize"]) && (int)( $this->pluginSettings["autocomplete_style"]["hoverResultFontSize"]) > 0)  ? $this->pluginSettings["autocomplete_style"]["hoverResultFontSize"] : null ,
					...
					"autocompleteResultHoverHighlightFontSize" => (isset($this->pluginSettings["autocomplete_style"]["hoverHighlightFontSize"]) && !is_null($this->pluginSettings["autocomplete_style"]["hoverHighlightFontSize"]) && (int)( $this->pluginSettings["autocomplete_style"]["hoverHighlightFontSize"]) > 0)  ? $this->pluginSettings["autocomplete_style"]["hoverHighlightFontSize"] : null ,
					...
					"descriptionFields" =>	is_array($this->fieldForExcerpt) ? $this->fieldForExcerpt: new stdClass(),
					...
					"descriptionSegments" => !empty($this->pluginSettings['descriptionSegments']) ? $this->pluginSettings['descriptionSegments']: self::DEFAULT_DESCRIPTION_SEGMENTS
				);
				...
				$saved_settings = get_option($this->pluginOptions["raw_settings"], '');
				if( !empty( $saved_settings ) ){
					foreach( $params["params"] as $paramKey => $paramValue ){
						$dataTypeSettingsParam = gettype($saved_settings->{$paramKey});
						$dataTypeParam = gettype($paramValue);
						if( property_exists( $saved_settings, $paramKey ) && !is_null($dataTypeSettingsParam) && strtolower($dataTypeSettingsParam) != "null" && !is_null($dataTypeParam) && strtolower($dataTypeParam) != "null" && $dataTypeParam != $dataTypeSettingsParam ){
							settype($params["params"][$paramKey], $dataTypeSettingsParam);
						}
					}
				}
				// END PRO PACK
	...
	protected function checkIfPostTypeCustomFieldsAlreadyExist( $postTypes = array() ){
		$allPostTypesExist = true;
		if( is_array($postTypes) && count( $postTypes ) > 0 ){
			foreach ($postTypes as $postType) {
				if (!array_key_exists($postType, $this->allCustomFields)) {
					$allPostTypesExist = false;
					break;
				}
			}
		}else{
		...
		return $allPostTypesExist;
	}

	protected function getAllCustomFields($postTypes = array()) {
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		if(!empty($this->allCustomFields) && $this->checkIfPostTypeCustomFieldsAlreadyExist( $postTypes )){
		...
		if(!empty($this->metaFieldsSkipped) && count($this->metaFieldsSkipped) > 0){
            $metaFieldsNotToBeQueriedNPT .= " `meta_key` NOT IN('".implode("','", $this->metaFieldsSkipped)."') ";
		...
		if(count($postTypes) == 0) {
			...
			if( is_array($allFields) && count( $allFields ) > 0 ){
				foreach( $allFields as $field ){
					if (substr($field, 0, 1) === '_') {
						array_push($systemFields, $field );
					}else{
						array_push($regularFields, $field);
					}
				}
			}
			$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
			...
			$allFields = $this->getAllMetaFieldsFromDB( $postTypes, $metaFieldsNotToBeQueried );
			foreach($postTypes as $key => $val){
				...
				$this->allCustomFields[$val] = $arrReturn[$val];
			}
			$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__, true);
			return $arrReturn;
		}
	...
	protected function getAllMetaFieldsFromDB( $postTypes = array(), $metaFieldsNotToBeQueried = ''){
		global $wpdb;
	...
	protected function savePostTypes($data, $sync = true ){
		 $this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		if($data != "") {
	...
	protected function getPostTypesForSearchSelection($postType = "", $onlyString = false){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$selection = isset($this->pluginSettings["siq_postTypesForSearchSelection"]) ? $this->pluginSettings["siq_postTypesForSearchSelection"]: '';
		$stringPostTypes = "";
		$arrPostTypes		= array();
	...
	protected function getBlackListUrls(){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$regExp = '/^https?:\/\/(www\.)?/';
        $pluginBlackListUrls = isset($this->pluginSettings["blacklist_urls"]) ? $this->pluginSettings["blacklist_urls"]: '';
	...
	private function recordActivation(){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$activationNumber = time().wp_rand(1,9999);
		$args			  = array();
	...
	public function getSearchPageUrl( $searchPage ){
		if( !$this->is_partner() ){
			...
			$post = is_numeric( $searchPage ) ? get_post( $searchPage ) : get_post(url_to_postid($searchPage));
			if( !is_null($post) && $post->post_status == 'publish' ){
				return $this->partnerSearchResultPageWithMacro(get_permalink( $post ));
			}else{
				return $this->partnerSearchResultPageWithMacro( $searchPage ); 
			}
		}
		return "";
	}
	...
	public function isAPIErrorLogEnabled() {
		return get_site_option('siq_api_error_log_enabled', false);
	}

	public function enableAPIErrorLog() {
		update_site_option('siq_api_error_log_enabled', true);
	}

	public function disableAPIErrorLog() {
		update_site_option('siq_api_error_log_enabled', false);
		$this->deleteAllAPIErrorLogRecords();
	}

	public function logAPIError(&$url, &$method, &$request, &$status_code, &$response) {
		if (!$this->isAPIErrorLogEnabled() && (!empty($this->pluginSettings['engine_code']) || (is_string($response) && strcmp($response, 'Domain already exists') == 0 )) ) {
			return;
		}
		global $wpdb;
	...
	protected function getPostTaxonomy( $postId, $taxonomy, $type ){
		$taxomonyList = array();
		if( is_array( $taxonomy ) && count( $taxonomy ) > 0 ){
			foreach( $taxonomy as $tax ){
				if( $tax->ID == $postId && $tax->taxonomy == $type ){
					$taxomonyList = explode( '<>', $tax->title );
					break;
				}
			}
		}
		if( count( $taxomonyList ) > 0 ){
			$taxomonyList = array_map([$this, 'handleSpecialChars'],$taxomonyList);
		}
		return $taxomonyList;
	}

	protected function processPostTaxonomies( $postId, $taxonomy, $excludeTaxonomies, $postData ){
		if( is_array( $taxonomy ) && count( $taxonomy ) > 0 ){
			foreach( $taxonomy as $tax ){
				if( $tax->ID == $postId && ( !is_array( $excludeTaxonomies ) || (is_array( $excludeTaxonomies ) && !in_array($tax->taxonomy, $excludeTaxonomies) ) ) ){
					$taxomonyList = explode( '<>', $tax->title );
					if( count( $taxomonyList ) > 0 ){
						$taxomonyList = array_map([$this, 'handleSpecialChars'],$taxomonyList);
						$taxonomyKey  = array_key_exists($tax->taxonomy, $this->taxomomyKeyMap) ? $this->taxomomyKeyMap[$tax->taxonomy]: $this->customTaxonomyPrefix . $tax->taxonomy;
						$postData[$taxonomyKey] = $taxomonyList;
					}
				}
			}
		}
	...
	public function logSyncProcessTime( $batch = 0, $queryTime = -1, $end = false, $last = false ){
		$option = $this->pluginOptions['siq_sync_process_log'];
		if( $batch == 0 && $end === false && $queryTime === -1 ){
			delete_site_option( $option );
		}
		$syncLog = get_site_option( $option, false);
		if( $syncLog !== false ){
			$syncLog = json_decode( $syncLog, true );
			if( array_key_exists( 'info', $syncLog ) && array_key_exists( $batch, $syncLog['info'] ) ){
				if( $end === false ){
					$syncLog['info'][$batch]['start'] = time();
				}else{
					$syncLog['info'][$batch]['end'] = time();
				}
			}else{
				$syncLog['info'][$batch] 	= array( "start" => time() );
			}
			if( $queryTime > -1 && isset($syncLog['info'][$batch]['start']) ){
				$syncLog['info'][$batch]['queryDelta'] = $queryTime;
			}
			if( isset( $syncLog['info'][$batch]['end'] ) && isset( $syncLog['info'][$batch]['start'] ) ){
				$delta = $syncLog['info'][$batch]['end'] - $syncLog['info'][$batch]['start'];
				$syncLog['info'][$batch]['delta'] = $delta; 
			}

			if( $last === true ){
				$syncLog['endtime'] 		= time();
				$syncLog['delta'] 			= time() - $syncLog['starttime'] ;
			}
		}else{
			$syncLog['starttime'] 		= time();
			$syncLog['info'][$batch] 	= array( "start" => time() );
		}

		$syncLog = json_encode( $syncLog );
		update_site_option( $option, $syncLog );
	}

	protected function saveErrorBatch( $batch, $postsPerBatch ){
		$option = $this->pluginOptions['siq_full_sync_failed_batches'];
		$failedBatches = get_site_option($option, false);
		if ($failedBatches !== false) {
			$failedBatches = json_decode($failedBatches, true);
		}else{
			$failedBatches = array();
		}
		if ( is_array($failedBatches) ) {
			$flag = false;
			foreach ($failedBatches as $key => $failed) {
				if ($failed['batch'] == $batch && $failed['posts_per_batch'] == $postsPerBatch) {
					$flag = true;
					break;
				}
			}
			if( $flag == false ){ array_push($failedBatches, array('batch' => $batch, 'posts_per_batch' => $postsPerBatch)); }
		}
		update_site_option($option, json_encode($failedBatches));
	}

	protected function deleteErrorBatchs(){
		$option = $this->pluginOptions['siq_full_sync_failed_batches'];
		delete_site_option($option);
	}

	protected function updateErrorBatch( $batch, $postsPerBatch ){
		$option = $this->pluginOptions['siq_full_sync_failed_batches'];
		$failedBatches = get_site_option($option, false);
		if ($failedBatches !== false) {
			$failedBatches = json_decode($failedBatches, true);
			if( is_array($failedBatches) && count($failedBatches) > 0 ){
				foreach( $failedBatches as $key => $failed ){
					if( $failed['batch'] == $batch && $failed['posts_per_batch'] == $postsPerBatch ){
						unset($failedBatches[$key]);
					}
				}
			}
			update_site_option($option, json_encode($failedBatches));
	...
	protected function checkIfFailedBatches(){
		$option = $this->pluginOptions['siq_full_sync_failed_batches'];
		$failedBatches = get_site_option($option, false);
		if ($failedBatches !== false) {
			$failedBatches = json_decode($failedBatches, true);
	...
	protected function checkIfPostIsInvalidVariableProduct( &$post ){
		if( $this->woocommerceActive && $post->post_type == 'product_variation' ){
			$postParent = get_post($post->post_parent);
			if( $postParent->post_status != 'publish' || strcmp($post->post_title, $postParent->post_title) == 0 ){
				return true;
			}
		}
		return false;
	...
	protected function createPermalinkForVariableProduct( &$document, &$post ){
		if( $document['documentType'] == 'product_variation' && strpos($document['url'], '?') === false && strpos($document['url'], 'attribute_pa') === false ){
			$excerpt = array_key_exists('excerpt', $document) ? $document['excerpt'] : '';
			$append	 = [];
			if (!empty($excerpt)) {
				$attributes = explode(',', $excerpt);
				if (is_array($attributes)) {
					foreach ($attributes as $attribute) {
						$attributeArr = array_map('trim', explode(':', $attribute));
						if (is_array($attributeArr) && count($attributeArr) == 2) {
							$append[] = 'attribute_pa_' . strtolower($attributeArr[0]) . '=' . strtolower($attributeArr[1]);
						}
					}
				}
				if (count($append) > 0) {
					$document['url'] .= '?' . implode('&', $append);
				}
			}
		}
	}

	protected function getChildPosts( $post_id ){
		global $wpdb;
		$allowedPostTypes = '';
	...
	protected function createDocumentsForPosts( $postIds = array() ){
		global $wpdb;
		if ( is_array($postIds) && count($postIds) > 0 ) {
			...
			if (!empty($response)) {
				...
				foreach ($response as $k => &$v) {
					...
					$document_ 	= $this->createDocumentFromPost($v, false, $dataTaxomony);
					if (is_array($document_) && count($document_) > 0) {
						$docsForSubmission[] = &$document_;
					}
				}
				return $docsForSubmission;
			}
	...
	protected function checkIfProductAndVariation($post, &$document, &$postBulkDocuments){
		if ($post->post_type == 'product' && is_array($this->postsToIndexAndSearch) && in_array('product_variation', $this->postsToIndexAndSearch)) {
			$variableProducts = $this->getChildPosts($post->ID);
			if (is_array($variableProducts) && count($variableProducts) > 0) {
				$variableProductsDocs = $this->createDocumentsForPosts($variableProducts);
				if( $postBulkDocuments === false ){
					$document = array_merge([$document], $variableProductsDocs);
				}else{
					$document = array_merge($document, $variableProductsDocs);
				}
				$postBulkDocuments = true;
	...
	private function truncateSyncTable(){
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		global $wpdb;
		$table_name = $this->syncTableName();
	...
	protected function checkIfPartner( $api_key ){
		global $siqAPIClient;
		$this->logFunctionCall(__FILE__, __CLASS__, __FUNCTION__, __LINE__);
		$result = array('partner' => false);
		if ( !empty($api_key) ) {
			try {
				$params['params'] = array('apiKey' => $api_key);
				$params['callMethod'] = 'GET';
				$params['callUrl'] = 'users/current';
				$this->log_error("starting api call: params(" . json_encode($params) . ")");
				$response = $siqAPIClient->makeAPICall($params, true);
				if ($response['response_code'] != "" && $response['response_body']['success'] == true) {
					if ($response['response_body'] != "" && count($response['response_body']) > 0) {
						...
						$result['response'] = $response['response_body'];
						$result['success'] = true;
						if( array_key_exists('publisher', $response['response_body']) && $response['response_body']['publisher'] == true ){
							$result['partner'] = true;
						}
					} else {
						$result['success'] = false;
	...
	public function is_partner(){
		$is_partner = get_option($this->pluginOptions['is_partner'], '');
		return $is_partner == 'yes' ? true : false;
	}
	protected function partnerSearchResultPageWithMacro( $resultPageUrl = '' ){
		$searchQueryMacro = $this->getSearchQueryParamName() . '={QUERY}';
		if (strpos($resultPageUrl, $searchQueryMacro) !== false) {
			return $resultPageUrl;
		} else {
			return $resultPageUrl . '?' . $searchQueryMacro;
		}
	}
}